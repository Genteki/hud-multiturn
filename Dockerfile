...
CMD ["sh", "-c", "uvicorn backend.agent:app --host 0.0.0.0 --port $AGENT_BACKEND_PORT --log-level warning >&2 & uvicorn backend.user:app --host 0.0.0.0 --port $USER_BACKEND_PORT --log-level warning >&2 & sleep 0.5 && python -m server.main"]
